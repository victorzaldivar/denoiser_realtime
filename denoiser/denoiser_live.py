...
def main():
    ...
    #model.load_state_dict(torch.load('e39_sudo_whamr_16k_enhnoisy_augment.pt', map_location=torch.device('mps')))
    ...
    print(f"Ready to process audio. Press CRT+C to stop the streaming")
    ...
    while True:
        try:
            ...
            if not args.bypass:
                ...
                frame = (frame - torch.mean(frame)) / torch.std(frame) # NORMALIZATION

                with torch.no_grad():
                    
                    out = model(frame.unsqueeze(1)).detach().squeeze()
                    #res = signal_distortion_ratio(out, frame)
